...
export class NeoContractClaims {
  static buildClaim(claimId: string, attestations: any, expires: number, verificationUri: string, wif: string, verbose: boolean = false): any {
    ...
    for (const attestation of attestations) {
      ...
      const payload = formatAttestation(attestation, verbose)
      attestationList.push(payload)
    }

    claimId = u.str2hexstring(claimId)
    attestationList.push('00' + hexLength(claimId) + claimId)
...
function formatAttestation(attestation: any, verbose: boolean = false): any {
  ...
  const fieldIsEncrypted = intToHexWithLengthPrefix(attestation.is_encrypted ? 1 : 0)
  ...
  const payload = 80 + u.int2hex(4) + '00' + fieldIsEncrypted + '00' + fieldIdentifier + '00' + fieldRemark + '00' + fieldValue
